...
import { lstatSync, readlinkSync, readdirSync, readFileSync as readProcFile } from "fs";
...
const INIT_TIMEOUT_MS = Math.max(120000, Number(process.env.WHATSAPP_INIT_TIMEOUT_MS || 180000));
...
const BRIDGE_PROBE_TIMEOUT_MS = Math.max(5000, Number(process.env.WHATSAPP_BRIDGE_PROBE_TIMEOUT_MS || 8000));
...
const BROWSER_CLOSE_TIMEOUT_MS = Math.max(5000, Number(process.env.WHATSAPP_BROWSER_CLOSE_TIMEOUT_MS || 15000));
const MAX_ACTIVE_BROWSERS = Math.max(1, Number(process.env.MAX_ACTIVE_WHATSAPP_BROWSERS || 2));
const BROWSER_SLOTS_KEY = "__whatmot_whatsapp_browser_slots";
...
const browserSlots = globalThis[BROWSER_SLOTS_KEY] || (globalThis[BROWSER_SLOTS_KEY] = {
  active: 0,
  waiters: [],
});

function acquireBrowserSlot(clientId) {
  if (browserSlots.active < MAX_ACTIVE_BROWSERS) {
    browserSlots.active += 1;
    return Promise.resolve(() => releaseBrowserSlot());
  }
  return new Promise((resolve) => browserSlots.waiters.push({ clientId, resolve }))
    .then(() => () => releaseBrowserSlot());
}

function releaseBrowserSlot() {
  const next = browserSlots.waiters.shift();
  if (next) {
    next.resolve();
    return;
  }
...
function releaseClientBrowserSlot(clientObj) {
  if (!clientObj.browserSlotHeld) return;
  clientObj.browserSlotHeld = false;
  releaseBrowserSlot();
}
...
function enqueueClientOperation(clientId, clientObj, operation, label = "operation") {
  clientObj.queueLength = (clientObj.queueLength || 0) + 1;
  const previous = clientObj.operationQueue || Promise.resolve();
  const current = previous.catch(() => {}).then(async () => {
    clientObj.queueLength = Math.max(0, clientObj.queueLength - 1);
    if (!clientObj.enabled && !["enable", "disable", "logout"].includes(label)) {
      throw new Error(`Client ${clientId} is disabled`);
    }
    clientObj.activeOperation = label;
    clientObj.operationStartedAt = new Date();
    try {
      const result = await operation();
      clientObj.lastSuccessfulBridgeOperation = new Date();
      return result;
    } finally {
      clientObj.activeOperation = null;
      clientObj.operationStartedAt = null;
    }
  });
...
async function probeClient(client, clientId = "unknown", clientObj = null, generation = null) {
  const page = client?.pupPage;
  const started = Date.now();
  const browserPid = client?.pupBrowser?.process?.()?.pid || null;
  const diagnostics = {
    clientId,
    generation,
    browserPid,
    ...
    durationMs: null,
    result: "failed",
    activeOperation: clientObj?.activeOperation || null,
    queueLength: clientObj?.queueLength || 0,
  };

  try {
    if (!page || page.isClosed?.()) throw new Error("WhatsApp Puppeteer page is closed");
    diagnostics.pageUrl = page.url?.() || null;

    // This is deliberately a local bridge/module probe, not a number lookup.
    // It verifies the exact internal modules used by getState()/getNumberId()
    // are installed and callable without performing a network query.
    const result = await withTimeout(page.evaluate(() => {
      ...
      const wid = widFactory?.createWid?.("0@c.us");
      return {
        state,
        socketReady: Boolean(socket),
        widFactoryReady: typeof widFactory?.createWid === "function" && Boolean(wid),
        queryExistsReady: typeof queryExists?.queryWidExists === "function",
      };
    }), BRIDGE_PROBE_TIMEOUT_MS, "WhatsApp bridge probe");

    if (result.state !== "CONNECTED") throw new Error(`WhatsApp bridge state is ${result.state || "unknown"}`);
    if (!result.socketReady || !result.widFactoryReady || !result.queryExistsReady) {
      throw new Error("WhatsApp Web internal bridge modules are not ready");
    }
    diagnostics.result = "healthy";
    diagnostics.durationMs = Date.now() - started;
    if (clientObj) {
      clientObj.bridgeState = "healthy";
      clientObj.lastBridgeProbe = diagnostics;
    }
    logger.info("WhatsApp bridge probe succeeded", diagnostics);
    return diagnostics;
  } catch (error) {
    diagnostics.durationMs = Date.now() - started;
    diagnostics.error = error.message;
    if (clientObj) {
      clientObj.bridgeState = "unhealthy";
      clientObj.lastBridgeProbe = diagnostics;
    }
    logger.warn("WhatsApp bridge probe failed", diagnostics);
    error.code = error.code || "BRIDGE_UNHEALTHY";
    error.bridgeDiagnostics = diagnostics;
    throw error;
  }
}
...
function chromiumProcessesUsingProfile(profileDir) {
  ...
  try {
    for (const entry of readdirSync("/proc")) {
      ...
      try {
        const cmdline = readProcFile(`/proc/${entry}/cmdline`, "utf8").replaceAll("\0", " ");
        if (/chrom(e|ium)|chrome_crashpad/i.test(cmdline) && cmdline.includes(profileDir)) {
          owners.push({ pid: Number(entry), cmdline: cmdline.trim() });
        }
      } catch {
    ...
    logger.warn("Unable to inspect /proc for Chromium profile ownership", { profileDir, err });
    return [{ pid: null, cmdline: "ownership check failed" }];
  }
  return owners;
}

function inspectChromiumProfileLocks(profileDir) {
  const names = ["SingletonLock", "SingletonCookie", "SingletonSocket", "SingletonSocket.lock", "DevToolsActivePort"];
  return names.map((name) => {
    ...
    try {
      ...
      if (stat.isSymbolicLink()) {
        try { target = readlinkSync(fullPath); } catch (err) { target = `readlink failed: ${err.message}`; }
      }
      return { name, path: fullPath, exists: true, type: stat.isSymbolicLink() ? "symlink" : "file", target };
    } catch (err) {
      return { name, path: fullPath, exists: false, reason: err.code || err.message };
    }
  });
}

function clearStaleChromiumProfileLockFiles(clientId) {
  const profileDir = getChromiumProfileDir(clientId);
  const detected = inspectChromiumProfileLocks(profileDir).filter((lock) => lock.exists);
  const owners = chromiumProcessesUsingProfile(profileDir);
  if (owners.length) {
    return { profileDir, detected, removed: [], owners, stale: false, reason: "active Chromium process references this profile" };
  }

  const removed = [];
  for (const lock of detected) {
    try {
        removeSync(lock.path);
        removed.push(lock.name);
    } catch (err) {
      logger.warn("Failed to remove stale Chromium profile artifact", { clientId, path: lock.path, err });
    }
  }

  // Recheck immediately before removal. This is deliberately conservative:
  // any uncertainty leaves the artifacts untouched for the next retry.
  const secondOwners = chromiumProcessesUsingProfile(profileDir);
  if (secondOwners.length) {
    return { profileDir, detected, removed: [], owners: secondOwners, stale: false, reason: "Chromium appeared during stale-lock recovery" };
  }
  ...
  return {
    ...
    removed,
    owners: [],
    stale: detected.length > 0 && removed.length === detected.length,
    reason: detected.length ? "artifacts existed but no Chromium process owned the profile" : "no profile lock artifacts detected",
  };
}

function getProfileLockDiagnostics(clientId) {
  const profileDir = getChromiumProfileDir(clientId);
  const detected = inspectChromiumProfileLocks(profileDir).filter((lock) => lock.exists);
  const owners = chromiumProcessesUsingProfile(profileDir);
  return {
    ...
    detected,
    owners,
    classification: owners.length ? "active" : detected.length ? "stale_or_leftover_metadata" : "clear",
  };
}
...
async function destroyClientInstance(clientId, clientObj, reason = "destroy") {
  ...
  try {
    ...
    if (browser) {
      try {
        await withTimeout(browser.close(), BROWSER_CLOSE_TIMEOUT_MS, "Chromium close");
      } catch (err) {
        clientLogger.warn("Failed to close Chromium after client destroy", {
          reason,
...
function recoverUnhealthyClient(clientId, clientObj, error) {
  if (!clientObj?.enabled || clientObj.lifecycleStopping) return;
  clientObj.bridgeState = "unhealthy";
  updateClientStatus(clientId, clientObj, {
    status: "degraded",
    error: error.message,
  ...
  scheduleReconnect(clientId, clientObj, "bridge_probe_failed");
}

async function sendWhatsAppMessage({ clientId, chatId, text, media = null, options = {} }) {
  const entry = clients[clientId];
  if (!entry?.client || !entry.ready || entry.bridgeState !== "healthy") {
    const error = new Error(`Client ${clientId} is unavailable or its WhatsApp bridge is unhealthy`);
    error.code = "CLIENT_UNAVAILABLE";
    throw error;
  }
  const instance = entry.client;
  const generation = entry.generation;
  const started = Date.now();
  try {
    const result = await enqueueClientOperation(clientId, entry, async () => {
    if (entry.client !== instance || entry.generation !== generation || !entry.ready) {
      const error = new Error(`Client ${clientId} changed state before sending`);
      error.code = "CLIENT_UNAVAILABLE";
      throw error;
    }
    await probeClient(instance, clientId, entry, generation);
    const registered = await withTimeout(instance.isRegisteredUser(chatId), OPERATION_TIMEOUT_MS, "WhatsApp number lookup");
    if (!registered) {
    ...
    if (media) {
      await withTimeout(instance.sendMessage(chatId, media, options), OPERATION_TIMEOUT_MS, "WhatsApp media send");
    } else {
      await withTimeout(instance.sendMessage(chatId, text), OPERATION_TIMEOUT_MS, "WhatsApp message send");
    }
    entry.lastSuccessfulSend = new Date();
    return { deliveryTime: Date.now() - started };
    }, "message_send");
    return result;
  } catch (error) {
    if (error.code === "BRIDGE_UNHEALTHY") recoverUnhealthyClient(clientId, entry, error);
    throw error;
  }
}
...
async function initClient(clientId, clientObj, options = {}) {
  ...
  const releaseBrowserSlotForClient = await acquireBrowserSlot(clientId);
  clientObj.browserSlotHeld = true;
  const generation = (clientObj.generation || 0) + 1;
  clientObj.generation = generation;
  clientObj.browserDiagnosticsAttached = false;
  clientObj.authenticatedGeneration = null;
  clientObj.readyGeneration = null;
  ...
  client.on("qr", async (qr) => {
    ...
    try {
      ...
      clientObj.lastQR = clientObj.qrGeneratedAt;
      updateClientStatus(clientId, clientObj, {
        status: "qr_ready",
        qrCode: qrDataUrl,
        ready: false,
  ...
  client.on("authenticated", () => {
    if (!isCurrent()) return;
    if (clientObj.authenticatedGeneration === generation) {
      clientLogger.warn("Duplicate authenticated event ignored", { generation });
      return;
    }
    clientObj.authenticatedGeneration = generation;
    ...
    clientObj.lastAuthentication = new Date();
    clientLogger.info("Authenticated successfully");
  });

  client.on("ready", async () => {
    if (!isCurrent()) return;
    if (clientObj.readyGeneration === generation) {
      clientLogger.warn("Duplicate ready event ignored", { generation });
      return;
    }
    clientObj.readyGeneration = generation;
    ...
    try {
      ...
      await enqueueClientOperation(
        clientId,
        clientObj,
        () => probeClient(client, clientId, clientObj, generation),
        "bridge_probe"
      );
    } catch (err) {
      if (!isCurrent()) return;
  ...
  function attachBrowserDiagnostics() {
    ...
    if (!browser || clientObj.browserDiagnosticsAttached) return;
    clientObj.browserDiagnosticsAttached = true;
    const browserProcess = browser.process?.();
    clientLogger.info("Chromium browser attached", {
      pid: browserProcess?.pid || null,
      profileDir: getChromiumProfileDir(clientId),
    });
    browser.on?.("disconnected", () => {
      clientLogger.error("Chromium browser disconnected", {
        pid: browserProcess?.pid || null,
        profileDir: getChromiumProfileDir(clientId),
      });
    });
    browserProcess?.once?.("exit", (code, signal) => {
      clientLogger.error("Chromium process exited", {
        pid: browserProcess.pid,
        code,
        signal,
        profileDir: getChromiumProfileDir(clientId),
      ...
      if (isCurrent()) {
        clientObj.bridgeState = "crashed";
        releaseClientBrowserSlot(clientObj);
      }
    });
  }
  ...
  client.on("disconnected", (reason) => {
    ...
    releaseClientBrowserSlot(clientObj);
    clientObj.lastBrowserDisconnect = new Date();
    clientObj.bridgeState = "disconnected";
    clientObj.initPromise = null;
    clientObj.initInProgress = false;
  ...
  const initPromise = Promise.race([
    ...
    .catch(async (err) => {
      if (isProfileLockLaunchError(err) && !lockRecoveryAttempted) {
        // Destroy this attempt first, then remove locks only after /proc confirms
        // no Chromium process owns this profile. Never blindly unlink locks.
        await destroyClientInstance(clientId, clientObj, "profile_lock_recovery");
        const recovery = clearStaleChromiumProfileLockFiles(clientId);
        clientLogger.warn("Detected Chromium profile lock", recovery);
        updateClientStatus(clientId, clientObj, {
          status: "error",
          error: recovery.stale
            ? "Recovered stale Chromium profile lock; retrying"
            : "WhatsApp profile is locked by another process",
          ready: false,
          qrCode: null,
        });
...
function buildClientProfile(clientKey, label) {
  return {
    ...
    lastUpdate: new Date(),
    name: label,
    profileDir: getChromiumProfileDir(clientKey),
    ...
    browserSlotHeld: false,
    bridgeState: "unknown",
    activeOperation: null,
    operationStartedAt: null,
    lastSuccessfulBridgeOperation: null,
    lastSuccessfulSend: null,
    lastBrowserDisconnect: null,
...
async function shutdownWhatsAppClients() {
  const destroys = [];
  for (const [clientId, clientObj] of Object.entries(clients)) {
    clearReconnectTimer(clientObj);
    destroys.push(destroyClientInstance(clientId, clientObj, "process_shutdown"));
  }
...
if (!global[SHUTDOWN_KEY]) {
  ...
  process.once("SIGTERM", () => { void shutdownWhatsAppClients(); });
  process.once("SIGINT", () => { void shutdownWhatsAppClients(); });
  process.once("beforeExit", flushClientStatesSync);
}

export { clients, initClient, startClient, toggleClient, logoutClient, sendWhatsAppMessage, getProfileLockDiagnostics, updateClientStatus, enqueueClientOperation, withTimeout, OPERATION_TIMEOUT_MS };
