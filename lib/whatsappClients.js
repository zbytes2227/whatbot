...
import { readdirSync, readFileSync as readProcFile } from "fs";
...
function clearStaleChromiumProfileLockFiles(clientId) {
  ...
  if (owners.length) {
    return { profileDir, removed: [], owners, stale: false };
  }
  ...
  if (secondOwners.length) {
    return { profileDir, removed: [], owners: secondOwners, stale: false };
  }

  return { profileDir, removed, owners: [], stale: removed.length > 0 };
}
...
export { clients, initClient, startClient, toggleClient, logoutClient, sendWhatsAppMessage, updateClientStatus, enqueueClientOperation, withTimeout, OPERATION_TIMEOUT_MS };
