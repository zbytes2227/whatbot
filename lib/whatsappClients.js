...
// Baileys emits high-volume protocol chatter at info/debug level. Keep only
// warnings and errors from the library; application lifecycle logs use logger.
const baileysLogger = createLogger({ module: 'baileys' }, { minLevel: 'warn' });
// Session storage is intentionally configured at runtime and must not be
// statically traced into the Next.js server bundle.
const SESSION_BASE_DIR = path.resolve(
  process.env.WHATSAPP_SESSION_PATH || path.join(/* turbopackIgnore: true */ process.cwd(), '.baileys_auth')
);
...
const savedStates = (() => { try { return existsSync(STATE_FILE) ? JSON.parse(readFileSync(STATE_FILE, 'utf8')) : {}; } catch (err) { logger.warn('Could not read WhatsApp state file', { error: err.message }); return {}; } })();
ensureDirSync(SESSION_BASE_DIR);

let stateSaveFailureLogged = false;
function saveStates() {
  try {
    writeFileSync(STATE_FILE, JSON.stringify(Object.fromEntries(Object.entries(clients).map(([id, c]) => [id, { enabled: c.enabled, accountInfo: c.accountInfo, status: c.status, ready: c.ready }])), null, 2));
    stateSaveFailureLogged = false;
  } catch (err) {
    if (!stateSaveFailureLogged) { stateSaveFailureLogged = true; logger.error('WhatsApp state persistence failed; check session volume permissions', { path: STATE_FILE, error: err.message }); }
  }
}
...
async function initClient(id, entry, { trigger = 'manual' } = {}) {
  ...
  entry.initPromise = (async () => {
    ...
    const { state, saveCreds } = await useMultiFileAuthState(path.join(/* turbopackIgnore: true */ SESSION_BASE_DIR, id));
    const { version } = await fetchLatestBaileysVersion().catch(() => ({ version: undefined }));
    const sock = makeWASocket({ version, auth: { creds: state.creds, keys: makeCacheableSignalKeyStore(state.keys, baileysLogger) }, browser: Browsers.ubuntu('WhatBot'), printQRInTerminal: false, markOnlineOnConnect: false, syncFullHistory: false, logger: baileysLogger });
    entry.client = sock;
    ...
    sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
      ...
      if (connection === 'close') {
        ...
        logger.warn('Baileys connection closed', { clientId: id, code: code || 'unknown', reason: loggedOut ? 'logged_out' : restarting ? 'restart_requested' : 'reconnect_required' });
